...
time_debug: 是否打印debug信息。
"""
def get_frame_from_file1(shared_array, shared_value, lock, frame_delay = 0, time_debug = False):
    global vid  
    global accum_time
    global curr_fps
    global prev_time
...
class frame_provider:
    ...
    def __init__(self, mode = 'file', file = None, ip = None, auto_expose = True, auto_balance = True):
        
        if mode == 'file':
            self.vid      = init_file(file)
    ...
    def read_file(self):
        while True:
            ...
            this_frame = get_frame_from_file(self.vid)
